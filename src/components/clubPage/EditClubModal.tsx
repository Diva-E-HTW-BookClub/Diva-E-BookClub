        ...
        await present({
            ...
            buttons: [
                ...
                },
                {
                    text: "Confirm",
                    handler: (value) => {
                        if(bookClubData && (value.maxMember.value < bookClubData.members.length)){
        ...
        <IonModal isOpen={isOpen}>
            <IonHeader>
                <IonToolbar>
                    ...
                    <IonButtons slot="end">
                        <IonButton form="editClub" type="submit">Save</IonButton>
                    </IonButtons>
                </IonToolbar>
            </IonHeader>
            <IonContent className="ion-padding">
                <form id="editClub" onSubmit={handleSubmit(submitData)}>
                    <IonItem className={errors.name ? 'ion-invalid' : 'ion-valid'}>
                        ...
                        <IonInput {...register("name", {required: "Please enter a Club Name"})}/>
                        {errors.name &&<IonNote slot="error" color={"danger"}>{errors.name.message}</IonNote>}
                    </IonItem>
                    <IonItem>
                        <IonLabel>Max number of members</IonLabel>
                        <IonChip onClick={() => openPicker()} {...register("maxMemberNumber")}>{maxMember}</IonChip>
                        <IonNote slot="helper">Cannot be less than current number of Members</IonNote>
                    </IonItem>
                    <IonItem lines="none">
                    <IonButton slot="start" fill="clear" onClick={() => deleteBookClub()} color="danger">
                        <IonIcon
                            slot="icon-only"
                            icon={trashOutline}
                        ></IonIcon>
