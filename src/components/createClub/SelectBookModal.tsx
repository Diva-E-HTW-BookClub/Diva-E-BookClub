...
const SelectBookModal = forwardRef<ModalHandle, SelectBookModalProps>(
  ({ setBook }: SelectBookModalProps, ref) => {
    ...
    async function searchBooks(q: string, offset: number) {
      ...
      const result = await fetch(
        `${api}?q=${q}&fields=title,author_name,cover_i&limit=20&offset=${offset}`
      );
    ...
    return (
      ...
      >
        <IonHeader>
          ...
          <IonToolbar>
            <IonSearchbar
              onClick={() => selectBookModal.current?.setCurrentBreakpoint(1)}
              class="custom ion-no-padding"
